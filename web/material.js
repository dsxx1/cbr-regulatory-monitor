...
renderStats=function(){baseStats();const cells=$('#stats').children;const ready=data.news.filter(n=>n.brief).length;const waiting=data.news.length-ready;
 ...
 progress.replaceChildren(el('strong','',`Разобрано ${ready} из ${data.news.length}`),el('span','','Архив обрабатывается порциями. Режим запуска указан сверху. '+(data.backlog?.last_finished?'Последняя порция: '+formatDate(data.backlog.last_finished,true)+' ЕКБ. ':'')),el('span','',`Ожидают повтора: ${counts.retry||0}. Нужна проверка причины: ${counts.needs_operator||0}.`));
